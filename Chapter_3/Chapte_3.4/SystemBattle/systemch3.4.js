...
let poisonTurns = 0;
let shieldTurns = 0;
let healingCount = 2; // Jumlah healing tersedia
...
function attemptEscape() {
    ...
    if (escapeChance <= 0.1) {
        ...
        window.location = "../../../Chapter_4_BOSS_1/Chapter4Dialog.html";
    } else {
...
function defend() {
    ...
    const spRestore = 15;
    playerSp = Math.min(playerSp + spRestore, 150); // maksimal SP 150
    ...
    // Tanda bahwa turn ini menggunakan defend (damage reduction 20%)
...
function useHealing() {
    ...
    const healAmount = 20;
    playerHp = Math.min(playerHp + healAmount, 100); // HP maksimal 100
    healingCount--;
...
function enemyAttack() {
    ...
    setTimeout(() => {
        ...
        if (shieldTurns > 0) {
            ...
            if (playerUsedDefend) {
                enemyDamage = Math.floor(enemyDamage * 0.8); // 20% reduction
...
function nextStage() {
    ...
    window.location = "../../../Chapter_4_BOSS_1/SystemBattle/BattleSystemCh4.html";
}
